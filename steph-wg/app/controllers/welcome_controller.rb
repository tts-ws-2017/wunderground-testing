class WelcomeController < ApplicationController
  def test
    response = HTTParty.get("http://api.wunderground.com/api/58f4a1c7ad0a1e95/geolookup/conditions/q/AZ/Phoenix.json")
         ...
         results = HTTParty.get("http://api.wunderground.com/api/#{Figaro.env.wunderground_api_key}/geolookup/conditions/q/#{params[:state]}/#{params[:city]}.json")
              ...
              #Checks to see if the response contains an array (ambigous response for an invalid city/state combination) or a hash(valid response)
                  if results.key?("location") 
                    # Store our location in the database if we don't already have it
                    if location_exists == false
                      dname = params[:city] + ", " + params[:state]
                      Location.create(city: params[:city], state: params[:state], dname: dname)
                    end
              ...
              @stored_locations = Location.order(:city)
  end
end
